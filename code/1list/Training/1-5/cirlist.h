...
template<class T>
void CirList<T>::josephus(int s, int m)
{
    if ((s > this->sz) || (this->sz == 0)
        || (m == 0)) {
        std::cout << "failed to solve josephus." << std::endl;
        return;
    }
    std::cout << "josephus answer: ";
    CirListNode<T> *temp1 = this->head, *temp2; // 前驱结点和删除结点

    for (int i = 0; i < s - 1; ++i) temp1 = temp1->next;
    int cnt = 0;
    ...
    while (this->sz) {
        ...
        if (cnt == m) {
            ...
            if (temp2 == rear) rear = temp1, temp1 = this->head;
            delete temp2;
            --this->sz;
            continue;
        }

        temp1 = temp1->next;                    // 前驱结点往后走

        if (temp1 == rear) temp1 = this->head;  // 到了尾结点循环回来
    }
    std::cout << std::endl;
}
