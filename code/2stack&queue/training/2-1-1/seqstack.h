...
template<class T>
SeqStack<T>::SeqStack(int sz) : ptr(-1), maxSize(sz), increaseSize(sz / 2) {
...
template<class T>
void SeqStack<T>::capacity() const {
    return this->maxSize;
}
...
// 不同于平常的函数重载模板泛型这样不会实例化编译器报错,问题参考;https://www.cnblogs.com/autosar/archive/2011/10/29/2228674.html
// template<class T>
